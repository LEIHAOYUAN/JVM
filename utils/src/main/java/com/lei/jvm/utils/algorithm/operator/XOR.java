package com.lei.jvm.utils.algorithm.operator;
...
public class XOR {
     ...
     * 找到eor里最右边一位为1的数，例如9（1001）和3（0011）异或为10（1010），寻找最右边一位1结果就是2（0010）。此时数组就被分为：某一位为1的和某一位不为1的，a，b分别在不同的组。
     * 3、最后，只需要将上述分组的某一组拉出来全异或（得到了eorr），便可回归1.2.1问题；另一个数仅需要将eor和eorr异或便可得到：因为a^a^b=b;b^a^b=a;
     */
