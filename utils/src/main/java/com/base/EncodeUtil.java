...
public final class EncodeUtil {
    ...
    public static String uriComponent(String value) {
        byte[] bytes = StringUtil.bytes(value);
    ...
    public static String decodeURIComponent(String value) {
        ...
        for (; index < length; index++) {
            ...
            if (ch == '%') {
                if ((index + 2) >= length)
                    throw new IllegalArgumentException("invalid uri encoding, value=" + value.substring(index));
    ...
    public static String base64(String value) {
        return base64(StringUtil.bytes(value));
    }
